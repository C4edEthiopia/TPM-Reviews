...
```{r setup}
#| include: false
# Interactive charts are built from TPM_data.xlsx at render time.
# Required packages: readxl, dplyr, tidyr, ggplot2, plotly, forcats, scales
library(readxl); library(dplyr); library(tidyr); library(ggplot2)
library(plotly); library(forcats); library(scales)

# Make every figure and every plotly widget fill 100% of its container width
# instead of knitr's fixed default (960px). This is the R-side control that
# stops charts from overflowing the slide/column, so no horizontal scrollbar.
...
intify <- function(p, tip = "text", h = 400, w = NULL) {
  ggplotly(p, tooltip = tip, height = h) |>
    config(displayModeBar = FALSE, responsive = TRUE) |>
    layout(autosize = TRUE,
           font = list(family = "Source Sans Pro, Calibri, sans-serif"))
}
...
qr <- read_excel(SRC, sheet = "quarter_reports") |>
  mutate(on_time = !is.na(`less than 1 month`),
         lab = sprintf("Q%d (%s)", Quarter,
                       ifelse(on_time, "within 1 month", "after 1 month")),
...
/* Static chart images fit within the slide or column, keeping aspect ratio,
   so they never require left-to-right scrolling. */
.reveal .slides img { max-width: 100%; height: auto; }
.reveal .panel-tabset img { max-width: 100%; max-height: 74vh; width: auto; height: auto; margin: 0 auto; display: block; }
.reveal .columns .column img { max-width: 100%; height: auto; max-height: 74vh; }
</style>
```
...
p <- ggplot(cap) +
  ...
  labs(x = "Months with a monitoring plan (of 10)", y = NULL) +
  theme_c4ed() +
...
p <- ggplot() +
  ...
  labs(x = "Share of weekly reports by submission day", y = NULL) +
  theme_c4ed() +
  theme(panel.grid.major.y = element_blank())
intify(p, w = 500)
```
...
p <- ggplot(mr_l) +
  geom_rect(aes(xmin = 0, xmax = mid, ymin = my - 0.31, ymax = my + 0.31,
                fill = bucket,
                text = sprintf("%s report: submitted %s after month end",
                               m, bucket))) +
...
p <- ggplot(qt) +
  geom_rect(aes(xmin = 0, xmax = mid, ymin = qy - 0.25, ymax = qy + 0.25,
                fill = on_time,
                text = sprintf("Q%d report (%d activities): %s",
                               Quarter, Activities, tlab))) +
  ...
  scale_y_continuous(breaks = qt$qy, labels = qt$ylab,
                     limits = c(0.4, 3)) +
  labs(x = "Time to submission after quarter end (bracket midpoint shown)",
       y = NULL) +
  theme_c4ed() +
  theme(panel.grid.major.y = element_blank())
intify(p, h = 430, w = 680)
...
#| fig-width: 13.2
#| fig-height: 5.2
plot_coverage_region("Bahir Dar")
```
...
p <- ggplot(agg) +
  ...
  labs(x = "Total staff travel days (person-days)", y = "Travel number") +
  theme_c4ed() +
  theme(panel.grid.major.y = element_blank())
intify(p, w = 680)
```
...
p <- ggplot(eq_tot,
  ...
  labs(x = "Units procured and distributed", y = NULL) +
  theme_c4ed() +
  theme(panel.grid.major.y = element_blank())
intify(p, w = 680)
```
...
p <- ggplot(bd_o2) +
 ...
 scale_x_continuous(
  ...
  labs(x = "Amount (project currency)", y = NULL) +
...
# Cap the left margin so the item-name gutter does not push the bars to the
# right. ggplotly would otherwise reserve ~324px for the longest label; 190px
# fits the labels and lets the bars use the reclaimed space on the left.
intify(p, w = 840) |>
...
p <- ggplot(cat_agg) +
  ...
  scale_y_continuous(breaks = cat_agg$cy, labels = cat_agg$name_pct,
                     limits = c(0.5, 3.75)) +
  labs(x = "Share of category budget", y = NULL) +
  theme_c4ed() +
  theme(panel.grid.major.y = element_blank())
intify(p, w = 680)
```
...
p <- ggplot(bd_p) +
  ...
  labs(x = "Share of line budget", y = NULL) +
  theme_c4ed() +
  theme(panel.grid.major.y = element_blank())
intify(p, w = 680)
```
