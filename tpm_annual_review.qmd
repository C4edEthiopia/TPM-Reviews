...
theme_c4ed <- function(base_size = 15) {
  theme_minimal(base_size = base_size) +
    theme(panel.grid.minor = element_blank(),
          panel.grid.major = element_line(colour = "#e2e8f0",
                                          linewidth = 0.4),
          axis.text = element_text(colour = TEXTC),
          axis.title = element_text(colour = TEXTC),
...
day_cols <- c(Monday = PRIMARY, Tuesday = MINT, Wednesday = BLUE)
wr2 <- wr |>
...
plot_monthly <- function() {
  if (!is.null(mr_actual)) {
    d <- mr_actual |>
      mutate(late = days > 10, m = factor(m, levels = rev(m)))
    p <- ggplot(d, aes(x = days, y = m, fill = late)) +   # horizontal bars
      geom_col(width = 0.72) +
      geom_text(aes(label = sprintf("%d days", as.integer(days))),
                hjust = -0.2, size = 4.6, colour = TEXTC) +
...
plot_quarterly <- function() {
  ...
  # append the exact submission days recorded in the sheet, when available
  base_lab <- ifelse(qt$on_time, "submitted within 1 month",
                     "submitted after 1 month")
  if (!is.null(qr_actual)) {
    dd <- qr_actual |> transmute(Quarter, days = as.integer(days))
    qt <- qt |> left_join(dd, by = "Quarter")
    qt$tlab <- ifelse(!is.na(qt$days),
                      sprintf("%s (%d days)", base_lab, qt$days), base_lab)
  } else {
    qt$tlab <- base_lab
  }
  p <- ggplot(qt) +
    ...
    scale_fill_manual(values = c(`TRUE` = PRIMARY, `FALSE` = WARN),
                      guide = "none") +
    scale_colour_identity() +
    scale_x_continuous(limits = c(0, 3.0), breaks = 0:2,
...
# ---- equipment value in US$ ----
# Vests, Laptops (3 x 900) and Printers (3 x 800) as advised by the Country
# Office; Tablets and E-bikes are taken from the budget sheet.
bd_val <- function(item) {
  v <- suppressWarnings(bd$Budget[bd$Item == item])
  if (length(v)) sum(v, na.rm = TRUE) else NA_real_
}
item_value <- c(Tablets  = bd_val("Tablets"),
                Vests    = 1500,
                Ebikes   = bd_val("E-bike buying"),
                Laptops  = 2700,
                Printers = 2400)
eq_val <- eq_tot |>
  mutate(value = as.numeric(item_value[as.character(item)]),
         units = as.integer(t),
         disp = ifelse(as.character(item) == "Printers",
                       "Printers & office supplies", as.character(item))) |>
  arrange(value) |>                                  # order by amount
  mutate(iy = row_number(),
         lab = sprintf("  %d %s  ($%s)  -  %s", units, disp,
                       formatC(value, format = "d", big.mark = ","), months))
...
# Takes the maximum FMA count assigned across surveys per region
# per month (the same field monitors cover several surveys; no summing), drawn as STACKED vertical bars (months on the x-axis, WFP sub
...
plot_pdm_bars <- function(h = 500) {
  d <- pdm_data |>
    ...
    summarise(FMAs = max(Monitors), .groups = "drop") |>
...
# PDM as dodged (grouped) bars: same data, sub offices side by side per month
plot_pdm_dodge <- function() {
  d <- pdm_data |>
    group_by(Month, Region) |>
    summarise(FMAs = max(Monitors), .groups = "drop") |>
    mutate(Month = factor(as.character(Month), levels = pdm_month_order))
  p <- ggplot(d, aes(Month, FMAs, fill = Region)) +
    geom_col(position = position_dodge2(preserve = "single", padding = 0.1),
             width = 0.85) +
    geom_text(aes(label = FMAs),
              position = position_dodge2(width = 0.85, preserve = "single"),
              vjust = -0.35, size = 4.2, fontface = "bold", colour = TEXTC) +
    scale_fill_manual(values = pdm_region_cols, name = "WFP sub office:") +
...
# coverage aggregated across activities: one line per WFP sub office over the
# months (all monitoring reports summed per office and month), single chart.
cov_office_cols <- c("Bahir Dar" = PRIMARY, "Dessie" = BLUE,
                     "Mekelle" = AMBER, "Asossa" = "#B23A48")
plot_coverage_office_lines <- function() {
  d <- cov_long |>
    group_by(Location, date, m) |>
    summarise(Value = sum(Value), .groups = "drop") |>
    mutate(Location = factor(Location, levels = cov_levels))
  grand <- sum(d$Value)
  p <- ggplot(d, aes(date, Value, colour = Location, group = Location)) +
    geom_line(linewidth = 1) +
    geom_point(size = 2) +
    scale_colour_manual(values = cov_office_cols, name = "WFP sub office:") +
    scale_x_date(date_labels = "%b-%y", date_breaks = "1 month") +
    scale_y_continuous(expand = expansion(mult = c(0.05, 0.12))) +
    labs(title = paste0("Grand total checklists submitted: ",
                        formatC(grand, big.mark = ",", format = "d")),
         x = NULL, y = "Checklists submitted") +
    theme_c4ed() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
plot_budget_overall <- function(hole = 0.35) {
  ...
  p <- ggplot(parts) +
    ...
    theme(legend.position = "bottom",
          legend.text = element_text(face = "bold"))
  # total budget shown in the centre of the donut hole
  if (hole > 0) {
    p <- p + annotate("text", x = 0, y = 0,
                      label = paste0("Total budget\n$",
                                     formatC(sum(bd$Budget), big.mark = ",",
                                             format = "d")),
                      size = 5.5, fontface = "bold", colour = TEXTC,
                      lineheight = 0.95)
  }
  p
}
...
### By sub office (grouped)

```{r}
plot_pdm_dodge()
```

### By sub office (stacked)
...
```{r}
dollar0 <- function(v) paste0("$", formatC(v, format = "d", big.mark = ","))
p <- ggplot(eq_val, aes(value, iy, fill = item)) +
  geom_rect(aes(xmin = 0, xmax = value, ymin = iy - 0.31, ymax = iy + 0.31),
            colour = GREYL, linewidth = 0.2) +
  geom_text(aes(x = value, label = lab),
            hjust = 0, size = 4.2, colour = TEXTC) +
  scale_fill_manual(values = item_cols, guide = "none") +
  scale_x_continuous(labels = dollar0, expand = expansion(mult = c(0, 0.5))) +
  scale_y_continuous(breaks = eq_val$iy, labels = eq_val$disp) +
  labs(x = NULL, y = NULL) +
  theme_c4ed() +
  theme(panel.grid.major.y = element_blank())
intify(p, w = 690)
...
fin_l <- fin |>
  ...
  pivot_longer(c(Expense, Forecast, Remainder),
