  ...
  if (!is.null(mr_actual)) {
    ...
    p <- ggplot(d, aes(days, my, fill = late)) +
      ...
      geom_text(aes(label = sprintf("%d days", as.integer(days))),
                hjust = -0.15, fontface = "bold", size = 4.4, colour = TEXTC) +
...
# quarterly timeliness: actual days after quarter end when available (30-day
# standard shown), else the legacy within/after one-month view.
plot_quarterly <- function() {
  if (!is.null(qr_actual)) {
    d <- qr_actual |>
      mutate(qy = rev(row_number()), late = days > 30,
             ylab = if ("Activities" %in% names(qr_actual))
                      sprintf("Q%d (%d activities)", Quarter, Activities)
                    else sprintf("Q%d", Quarter))
    p <- ggplot(d, aes(days, qy, fill = late)) +
      geom_col(width = 0.5) +
      geom_text(aes(label = sprintf("%d days", as.integer(days))),
                hjust = -0.15, fontface = "bold", size = 4.4, colour = TEXTC) +
      geom_vline(xintercept = 30, colour = SECONDARY, linetype = "dashed",
                 linewidth = 0.8) +
      scale_fill_manual(values = c(`FALSE` = PRIMARY, `TRUE` = WARN),
                        breaks = c(FALSE, TRUE),
                        labels = c("Within 1 month", "After 1 month"),
                        name = NULL) +
      scale_x_continuous(expand = expansion(mult = c(0, 0.2))) +
      scale_y_continuous(breaks = d$qy, labels = d$ylab) +
      labs(x = NULL, y = NULL) +
      theme_c4ed() + theme(panel.grid.major.y = element_blank())
    return(intify(p))
  }
...
plot_budget_overall <- function(hole = 0.35) {
  ...
  ggplot(parts) +
    ...
    geom_segment(data = function(d) d[d$small, ],
                 aes(x = 5.0, xend = 4.05, y = mid, yend = mid),
                 arrow = arrow(length = unit(0.22, "cm")),
                 linewidth = 0.8, colour = TEXTC) +
    geom_text(data = function(d) d[d$small, ],
              aes(x = 5.6, y = mid, label = outlab),
...
## Recap of key discussion points from the initial meeting

- **Recruitment & Deployment**
    - **Team Formed:** Successfully recruited a total of 68 monitors, including 3 Regional Monitoring Coordinators (RMCs) and 65 Field Monitoring Assistants (FMAs).
    - **Overcoming Friction:** Navigated hiring challenges, including high salary expectations, applicant unavailability, and rigid geographic preferences.
- **Quality Assurance & Partnerships**
    - **Two-Pillar Performance Strategy:** Implemented a data quality framework pairing direct RMC supervision and guidance/troubleshooting by Management and Support staff.
    - **Rotation:** Essential strategy to keep monitors alerted and reduce risk.
    - **Alignment:** Coordination meetings with regional WFP Area and Sub-Offices to align monitoring schedules and ensure field access clearances.
    - **Performance Evaluation:** we periodically evaluate our monitors and coordinators performance to improve quality of work and increase coverage.
- **Immediate Priorities**
    - Draft and align September-December monitoring plans.
    - Strengthen stakeholder collaboration to ensure seamless field operations.

## Progress update since the last meeting

- Recruitment
    - Area Monitoring Coordinators (2): Dessie and Gonder
    - Regional Monitoring Coordinators: 1 each for Amhara, Tigray and Benishangul-Gumuz (reassigned)
    - Field Monitoring Assistants (FMA): newcomers because of some changes
- Laptop/printers, visibility items and e-bikes procured and distributed
- Collaboration strengthened
    - Direct communication with RRS, LoA under preparation (our FMAs still have access to refugee camps)
- Training provided (including continuous refresher training): Monitor skills enhanced
